var _ = require('md_mudoc-lite/static/js/underscore');
// CommonJS
if (typeof require != 'undefined') {
  if (typeof Changeset == 'undefined') {
    Changeset = require('md_mudoc-lite/static/js/Changeset');
  }
}
...
exports.acePostWriteDomLineHTML = function (hook_name, args, cb) {
  ...
  if (node.children.length > 0) {
    ...
    for (var i = 1; i < node.children.length; i++) {
      ...
      innerHTML = innerHTML.replace(/(<.+>)","/gi, '","$1');
  ...
  try {
    ...
    if (Datatables.isFocused()) {
      ...
      if (
        !specialHandled &&
        isTypeForSpecialKey &&
        ((type == 'keydown' && keyCode == Datatables.vars.JS_KEY_CODE_DEL) ||
          keyCode == Datatables.vars.JS_KEY_CODE_BS ||
          (String.fromCharCode(which).toLowerCase() == 'h' && evt.ctrlKey))
      ) {
  ...
  var Datatables = (function () {
      ...
      for (var rIndex = 0, rLen = payload.length; rIndex < rLen; rIndex++) {
        ...
        for (var tIndex = 0, tLen = tds.length; tIndex < tLen; tIndex++) {
          ...
          if (payloadSum >= payloadOffset) {
            ...
            var leftOverTdTxtLen =
              payloadSum - payloadOffset == 0
    ...
    dt.addTable = function (tableObj) {
      ...
      if (xByYSelect != null && xByYSelect.length == 3) {
        ...
        for (var i = 0; i < rows; i++) {
          ...
          if (i == 0) {
            this.performDocApplyTblAttrToRow(
              rep.selStart,
              this.createDefaultTblProperties(null, true),
            );
          } else {
            this.performDocApplyTblAttrToRow(
              rep.selStart,
              this.createDefaultTblProperties(authors),
            );
          }
        }
        this.context.editorInfo.ace_doReturnKey();
    ...
    dt.insertTblRow = function (aboveOrBelow) {
      ...
      try {
        ...
        if (aboveOrBelow == 'addA') {
          ...
          this.performDocApplyTblAttrToRow(
            rep.selStart,
            this.createDefaultTblProperties(null, isFirstRow),
          );
        } else {
          //below curr row ( aboveOrBelow = 'addB')
          this.insertTblRowBelow(payload[0].length);
    ...
    dt.deleteTable = function () {
      ...
      try {
        ...
        while (rep.lines.atIndex(line).text.indexOf('data-tables') != -1) {
          //count num of rows above current pos
          numOfLinesAbove++;
          line--;
        }
        line = rep.selEnd[0] + 1;
        while (rep.lines.atIndex(line).text.indexOf('data-tables') != -1) {
          //count num of rows below current pos
          numOfLinesBelow++;
          line++;
        }
        rep.selStart[1] = 0;
    ...
    dt.updateTableProperties = function (props) {
      ...
      if (
        props.tblWidth ||
        props.tblHeight ||
        props.tblBorderWidth ||
        props.tblBorderColor ||
        props.tblColWidth ||
        props.tblSingleColBgColor ||
        props.tblEvenRowBgColor ||
        props.tblOddRowBgColor ||
        props.tblColVAlign
      ) {
        var start = [];
        (start[0] = rep.selStart[0]), (start[1] = rep.selStart[1]);
        ...
        tempStart[1] = start[1];
        while (
          tempStart[0] < rep.lines.length() &&
          rep.lines.atIndex(tempStart[0]).text.indexOf('data-tables') != -1
        ) {
          //start from top of a table
    ...
    dt.addRowAttr = function (tblJSONObj, tblProperties, attrName, attrValue) {
      ...
      if (attrName == 'bgColor') {
        ...
        //even-odd rows properties,rowAlign
    ...
    dt.updateTablePropertiesHelper = function (props, start, currTd) {
      ...
      try {
        ...
        if (props.tblWidth || props.tblHeight || props.tblBorderWidth || props.tblBorderColor) {
          ...
          if (
            props.attrValue != null &&
            (typeof currAttrValue == 'undefined' || currAttrValue != props.attrValue)
          ) {
        ...
        if (
          props.tblCellFont ||
          props.tblCellFontSize ||
          props.tblCellBgColor ||
          props.tblCellHeight ||
          props.tblcellVAlign ||
          props.tblCellBold ||
          props.tblCellItalic ||
          props.tblCellDecoration ||
          props.tblCellVAlign ||
          props.tblCellHAlign
        ) {
          var tblProps = this.addCellAttr(
            start,
            tblJSONObj,
            tblProperties,
            props.attrName,
        ...
        if (props.tblColWidth || props.tblSingleColBgColor || props.tblColVAlign) {
          var tblProps = this.addColumnAttr(
            start,
            tblJSONObj,
            tblProperties,
            props.attrName,
    ...
    dt.insertTblRowBelow = function (numOfRows, table) {
      ...
      rep.selEnd[1] = rep.selStart[1] = currLineText.length;
      this.context.editorInfo.ace_doReturnKey();
      this.context.editorInfo.ace_performDocumentReplaceRange(
        rep.selStart,
        rep.selEnd,
      ...
      try {
        ...
        if (typeof payload[currRow][currTd + 1] == 'undefined') {
          ...
          if (
            nextLineText == null ||
            nextLineText == '' ||
            nextLineText.indexOf('data-tables') == -1
          ) {
            //create new row and move caret to this new row
            this.insertTblRowBelow(null, null);
            this.performDocApplyTblAttrToRow(rep.selStart, this.createDefaultTblProperties());
            rep.selEnd[1] = rep.selStart[1] = this.vars.OVERHEAD_LEN_PRE;
            updateEvenOddBgColor = true;
          } else {
            //just move caret to existing next row
            currTd = -1;
            rep.selStart[0] = rep.selEnd[0] = rep.selStart[0] + 1;
          ...
          //tab to the next col and update cell user color
          var nextTdTxtLen =
            typeof payload[currRow] == 'undefined'
              ? -leftOverTdTxtLen
    ...
    dt.insertTblColumn = function (leftOrRight, start, end) {
      ...
      try {
        ...
        while (
          rep.selStart[0] < rep.lines.length() &&
          rep.lines.atIndex(rep.selStart[0]).text.indexOf('data-tables') != -1
        ) {
          //count num of rows above current pos
          var currLineText = rep.lines.atIndex(rep.selStart[0]).text;
          var tblJSONObj = JSON.parse(currLineText);
    ...
    dt.insertTblRowBelow = function (numOfRows, table) {
      ...
      rep.selEnd[1] = rep.selStart[1] = currLineText.length;
      this.context.editorInfo.ace_inCallStackIfNecessary(
        'newline',
        this.context.editorInfo.ace_doReturnKey,
      );
      context.editorInfo.ace_performDocumentReplaceRange(
        rep.selStart,
        rep.selEnd,
        JSON.stringify(tableObj),
      );
    ...
    dt.doReturnKey = function () {
      ...
      if (currLineText.indexOf('data-tables') == -1) return false;
        ...
        try {
          ...
          else if (currLineText.substring(currCarretPos - 2, currCarretPos + 1) == '","')
            return true;
          ...
          try {
            var jsonObj = JSON.parse(
              currLineText.substring(0, start[1]) + newText + currLineText.substring(start[1]),
            );
      ...
      try {
        ...
        switch (keyCode) {
          case this.vars.JS_KEY_CODE_BS:
            if (
              cellEntryLen != 0 &&
              cellEntryLen > currTdInfo.leftOverTdTxtLen - this.vars.OVERHEAD_LEN_MID
            ) {
      ...
      if (point.node == root) {
        ...
        while ((parNode = n.parentNode) != root) {
          if ((prevSib = n.previousSibling)) {
            n = prevSib;
            var textLen =
              nodeText(n).length == 0 ? this.nodeTextPlain(n).length : nodeText(n).length;
            col += textLen;
          } else {
            n = parNode;
          }
              ...
              if (docChar > 0) {
                if (evt.metaKey || evt.ctrlKey || evt.altKey) {
                  ...
                  while (
                    ...
                    editorInfo.ace_isWordChar(rep.alltext.charAt(deleteBackTo - 1))
                  ) {
                    deleteBackTo--;
                  }
                  ...
                  editorInfo.ace_performDocumentReplaceCharRange(
                    docChar - 1 - returnKeyWitinTblOffset,
                    docChar,
                    '',
                  );
                }
    ...
    dt.getCurrTblOddEvenRowBgColor = function (startRowNum, currRowNum) {
      ...
      if (startRowNum != currRowNum) {
        ...
        rowBgColors.evenBgColor =
          jsoTblProp1['rowAttrs']['evenBgColor'] || jsoTblProp2['rowAttrs']['evenBgColor'];
        rowBgColors.oddBgColor =
          jsoTblProp1['rowAttrs']['oddBgColor'] || jsoTblProp2['rowAttrs']['oddBgColor'];
      }
      return rowBgColors;
    };
    ...
    dt.sanitizeTblProperties = function (
      start,
